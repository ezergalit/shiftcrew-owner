...
- משקה אלכוהולי: "אלכוהול" ב-pregnancy **וגם** ב-pitfalls. בירה ⇒ גם גלוטן.
...
"דגים" אלרגיה; "דג נא" אזהרת הריון; "כוסברה" העדפה. אל תערבב.
⚠ ערכי ארבע הקבוצות האלה הם **רשימה סגורה בעברית** — הם היחידים בכל ה-JSON שאינם בשפת התפריט. מנה בשם \`Spicy Tuna Roll\` מקבלת \`allergens: ["דגים"]\`, לא \`["fish"]\`.

**allergens** — אך ורק: גלוטן, חלב, ביצים, אגוזים, בוטנים, דגים, רכיכות, סויה, שומשום.
שדה בטיחותי — עדיף ריק מאשר שגוי. סמן רק על סמך מרכיב מפורש **במנה הזו**. ירקות וכבושים אינם אגוזים.

**pregnancy** — אך ורק: דג נא, בשר נא, ביצה חיה, גבינה לא מפוסטרת, חלב לא מפוסטר, דגים עתירי כספית, נבטים חיים, כבד, אלכוהול.
...
**pitfalls** — אך ורק: כוסברה, חריף, שום, בצל, ג'ינג'ר, וסאבי, מיונז, טחינה, אלכוהול, זיתים, פטריות, חמוצים, גבינה כחולה.
...
const FLAG_VALUES: Record<string, Set<string>> = {
  allergens: new Set(["גלוטן", "חלב", "ביצים", "אגוזים", "בוטנים", "דגים", "רכיכות", "סויה", "שומשום"]),
  pregnancy: new Set(["דג נא", "בשר נא", "ביצה חיה", "גבינה לא מפוסטרת", "חלב לא מפוסטר", "דגים עתירי כספית", "נבטים חיים", "כבד", "אלכוהול"]),
  pitfalls:  new Set(["כוסברה", "חריף", "שום", "בצל", "ג'ינג'ר", "וסאבי", "מיונז", "טחינה", "אלכוהול", "זיתים", "פטריות", "חמוצים", "גבינה כחולה"]),
