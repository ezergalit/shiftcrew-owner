...
import {
  ChevronRight, Check, Coffee, UtensilsCrossed,
  Wine, Croissant, Bell, Users, Sparkles, Store, MapPin,
  ArrowLeft, ShieldCheck, CalendarDays, PenLine, Info,
  Loader2,
} from "lucide-react";
import { scOwner } from "../lib/shiftcrew";
import { savePositions, newPosition, MODEL_STAGGER } from "../lib/positions";
...
const TOTAL_STEPS = 3; // config steps (1..3); 0 = welcome, 4 = done
...
export default function SetupWizard({ restaurant, ownerName, onComplete }) {
  ...
  // Owner-defined positions, each with its own staffing style. We seed ONE waiter
  // position (staggered) so the step isn't blank and demonstrates the model — the
  // owner adds the rest (managers, kitchen, host…) or deletes this one.
  const [positions, setPositions] = useState(() => [newPosition("מלצרים", MODEL_STAGGER, 0)]);
  ...
  async function persistSetup() {
    ...
    try {
      ...
      // Replace-all the owner's positions (each holds its own JSONB staffing config).
      await savePositions(rid, positions);
      ...
      <Shell>
        <div className="flex-1 flex flex-col items-center justify-center px-7 text-center">
          ...
          <p className="text-sm text-gray-400 mt-3 leading-relaxed max-w-xs">
            ...
            ב-3 שלבים קצרים נגדיר את התפקידים שלך, איך מאיישים כל אחד, ומה לעשות כשחסר.
          </p>
          <div className="mt-7 w-full max-w-xs space-y-2.5 text-right">
            {[
              [Users, "תפקיד לכל סוג עובד/ת"],
              [CalendarDays, "איוש לפי הסגנון של כל תפקיד"],
      ...
      <Shell>
        <div className="flex-1 flex flex-col items-center justify-center px-7 text-center">
          ...
          <p className="text-sm text-gray-400 mt-2 leading-relaxed max-w-xs">
            {(name || "המסעדה")} מוגדרת עם {positions.length} תפקידים ואופן איוש לכל אחד. מהרגע הזה
  ...
  const stepValid =
    ...
    step === 3 ? (policy !== "custom" || customRule.trim().length > 0) :
    true;
  ...
  return (
    <Shell>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5">
        ...
        {step === 2 && (
          <>
            <StepTitle title="התפקידים שלך" sub="הגדר/י כל תפקיד ובחר/י איך מאיישים אותו — לפי משמרות קבועות או כניסה מדורגת" />
