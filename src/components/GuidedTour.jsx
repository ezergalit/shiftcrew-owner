...
const buildSteps = (teamCode) => [
  ...
  {
    tab: "tasks",
    title: "טאב המשימות — המשמרת של הצוות",
    body: "כאן בונים את מה שהצוות רואה כרשימה מסומנת ליום: צ׳קליסט פתיחה, כללי שירות במהלך המשמרת, צ׳קליסט סגירה ומשימות לימוד. לא צריך לכתוב — פותחים קבוצה, ״הוספה מהספרייה״ ומסמנים מה רלוונטי אצלכם. כל משימה ניתנת לעריכה, להסתרה זמנית (👁) ולסידור מחדש, ומתחתיה תראו כמה עובדים כבר סימנו אותה היום.",
  },
  {
    tab: "home",
    title: "מעקב — מי למד ומי צריך תזכורת",
    body: "חשוב להבין: המשחקים הם רק האימון — המטרה היא שהצוות יעבור את המבחנים, ובסוף את מבחן התפריט המלא (כ-40 שאלות עם שעון). בבית, מתחת לעדכון, שלושה כרטיסים שנפתחים בלחיצה: מי קרא את העדכון, ״מי למד היום״ — כולל מי נכנס ולא למד (הקבוצה שהכי שווה לשים לב אליה), ו״התקדמות ומבחנים״ — אחוז הידע של כל מלצר, המנות שהוא טועה בהן, ציוני המבחנים וגרף השיפור.",
  },
  {
    tab: "settings",
    title: "כך מוסיפים חברי צוות",
    body: `אין טפסים: בהגדרות → ״הצוות שלי״ יש קוד הצטרפות${teamCode ? ` (${teamCode})` : ""} — שתפו אותו עם המלצרים, למשל בקבוצת הוואטסאפ. כל אחד מוריד את אפליקציית הצוות, מזין את הקוד ואת שמו — וזהו, הוא בפנים ומתחיל ללמוד. באותו מקום תראו את כל מי שהצטרף, ותוכלו להסיר מי שעזב.`,
  },
  {
    tab: "settings",
