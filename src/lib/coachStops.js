...
export function itemNoun(items) {
  ...
  if (all((c) => c.knowledge)) return { one: "נושא", many: "נושאים", tap: "הקשה על נושא פותחת אותו לעיון" };
  if (all((c) => c.event)) return { one: "פריט", many: "פריטים", tap: "הקשה פותחת את כל הפרטים" };
  if (all((c) => c.drink === "סיגר")) return { one: "סיגר", many: "סיגרים", tap: "הקשה על סיגר פותחת אותו לעיון" };
  if (all((c) => c.drink)) return { one: "משקה", many: "משקאות", tap: "הקשה על משקה פותחת אותו לעיון" };
  return { one: "מנה", many: "מנות", tap: "הקשה על מנה פותחת אותה לעיון" };
}
...
export const STOPS = {
  ...
  "menu-cat": ({ cat, items }) => {
    ...
    // הצבעים נאמרים רק כשיש מה לצבוע — בקטגוריית אירועים אין אף אזהרה.
    const colours = (items || []).some(hasWarning) ? " הצבעים למעלה הם האזהרות שהצוות רואה." : "";
    return `${cat ? cat + " — " : ""}${n.tap}.${colours}`;
  },
  "menu-dish": ({ items }) => {
    const n = itemNoun(items);
    const seen = n.one === "פריט" ? "את כל הפרטים" : `את ה${n.one} כמו שהצוות רואה ${n.one === "מנה" ? "אותה" : "אותו"}`;
    return `כאן רואים ${seen}. «עריכת המנה» פותחת את השדות, וכל שינוי מגיע אליהם מיד.`;
  },
  "menu-end": ({ cat }) => (cat
