...
// to hospital.
//
// ⚠️ MIRRORED in shiftcrew-waiter/src/lib/dishFlags.js. Two repos, no shared package —
...
export const FLAG_GROUPS = [
  ...
  },
  {
    ...
    column: "pregnancy",
    recommended: true,
    // ⚠️ בלי «חלב לא מפוסטר» (יותם, 31.8) — חלב הוא לקטוז וחי באלרגיות;
    // גבינה לא מפוסטרת נשארת, כי היא סיכון הריון בפני עצמו.
    values: [
      "דג נא", "בשר נא", "ביצה חיה", "גבינה לא מפוסטרת",
  ...
  },
  {
    ...
    column: "pitfalls",
    recommended: true,
    // ⚠️ הרשימה קוצרה (יותם, 31.8): זיתים, פטריות, חמוצים, טחינה ואלכוהול
    // אינם מוקשים — ואלכוהול ממילא חי ב«רגישות בהריון», שם מקומו היחיד
    // (הוא הופיע פעמיים בכל משקה במסעדה ממוזגת-אזהרות).
    values: [
      "כוסברה", "חריף", "שום", "בצל", "ג'ינג'ר", "וסאבי", "מיונז", "גבינה כחולה",
    ],
  },
  {
    key: "kashrut",
